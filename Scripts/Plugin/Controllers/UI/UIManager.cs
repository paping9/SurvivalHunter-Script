...
namespace UI
{
    ...
    {
        public const float DefaultWidth        = 720.0f;
        public const float DefaultHeight       = 1280.0f;
        ...
        private void Start()
        {
            var fRateWidth        = (float)Screen.width / DefaultWidth;
            var fRateHeight       = (float)Screen.height / DefaultHeight;

            var fUIWidth          = DefaultWidth;
            var fUIHeight         = DefaultHeight;

            if (fRateWidth < fRateHeight)
            {
                _canvasScaler.matchWidthOrHeight = 0.0f;
                fUIHeight = DefaultWidth / (float)Screen.width * (float)Screen.height;
            }
            else
            {
                _canvasScaler.matchWidthOrHeight = 1.0f;
                fUIWidth = DefaultHeight / (float)Screen.height * (float)Screen.width;
            }
        ...
        private async UniTask<UIBase> CreateInstanceBasic(UiData uiData)
        {
            ...
            {
                var handle = _addressableManager.LoadInstantiate(uiData.UiId.Name, null);
